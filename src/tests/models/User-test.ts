...
describe("User Model", () => {
  ...
  describe("comparePassword", () => {
    ...
    it("should handle bcrypt errors", async () => {
      ...
      await expect(user.comparePassword(candidatePassword)).rejects.toThrow(
        "Bcrypt error"
      );
    });
  });
  ...
  describe("hashPassword", () => {
    ...
    it("should handle hashing errors", async () => {
      ...
      await expect(user.hashPassword(password)).rejects.toThrow(
        "Hashing error"
      );
    });
  });
  ...
  describe("Model hooks simulation", () => {
    it("should hash password before create", async () => {
      ...
      if ((userData as any).password) {
        mockUser.passwordHash = await mockUser.hashPassword(
